    ...
    while True:
        if __isRunning:        
            if detect_color != 'None' and start_pick_up:  #如果检测到方块,开始夹取
                ...
                time.sleep(1.5)
                AK.setPitchRangeMoving((0, 13, -3), -200,-90, 90, 1500) # 机械臂抬起来
                time.sleep(1.5)
                if not __isRunning:
                    continue
                Board.setPWMServoPulse(3 ,1050, 500) # 调整爪子角度找到最合适的值
                
                time.sleep(1.5)
                Board.setPWMServoPulse(1, 800, 500) # 闭合爪子
                time.sleep(1.5)
                ...
                result = AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 0), -90, -90, 0) # 运行到对应颜色的坐标上方
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color]), -180, -90, 0, 500)  # 放置到检测到颜色对应的坐标
                ...
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 8), -180, -90, 0, 800) # 运行到对应颜色的坐标上方
                time.sleep(0.8)
                if not __isRunning:
                    continue
                Board.setPWMServosPulse([1200, 4, 1,2500, 3,515, 4,2170, 5,945]) # 机械臂进行复位
                time.sleep(1.2)
                if detect_color == 'red':
